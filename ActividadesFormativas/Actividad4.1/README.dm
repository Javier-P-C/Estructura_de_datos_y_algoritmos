...
DFS:
  La función es recursiva y cuenta con un for para checar todos los nodos adyacentes. La recursión se realiza la cantidad de vértices que tiene el grafo y el for se realiza en total el mismo número de veces que el número de arcos. Por lo tanto se dice que el DFS es O(V*E); siendo v los vértices y e los arcos. Se puede decir también que es O(n^2) En este caso se hace otro loop para sacar el camino más corto que se guarda en el stack, el camino en el peor de los casos es O(n). En total se tiene una complejidad de O(n^2+n).

BFS:
  En primera, se recorre un arreglo de valores booleanos de n dimensiones, O(n). Después se tiene un for anidado a otro for,estos se utilizan para comprobar si ya se ha visitado cada vértice adyacente (Edges) de cada vértice (vértices); O(V+E). Después se llama a otra función para sacar el camino más corto por medio de un queue. Los valores del queue son metidos en un arreglo por un proceso O(n). Este arreglo es recorrido para encontrar el camino más corto, el cual en el peor de los casos es O(n). En total se tiene una complejidad de O((n^2)+3n), o solo O(n^2) si se simplifica.